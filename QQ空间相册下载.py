...
def picdownload():
    ...
    for img_url in img:  # 下载图片
        try:
            ...
            pass
        img_rel = img_ori.replace(r'/m/', r'/b/').replace(r'psbe?', r'psb?').replace(r'/mnull', r'/b').replace(r'/m&', r'/b&').replace(r'rf=photolist', r'rf=viewer_4')  # 很重要
        fileName = imgNames[imgPageCnt].getText().strip()
        imgPageCnt = imgPageCnt + 1
        print("当前图片："+fileName+'.jpg')
        if os.path.exists(fileName+'.jpg'):
            print("已存在")
            continue
        img_current = img_current + 1
        print("[{}/{}] Img_URL: {}".format(img_current, total_img, img_rel))
